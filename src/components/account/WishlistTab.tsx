      ...
      {wishlistedProducts.length === 0 ? (
        <div className="py-[60px] md:py-[80px] text-center bg-white rounded-[16px] md:rounded-[20px] border-2 border-dashed border-[#e5e5e5] px-[16px]">
          <div className="w-[48px] md:w-[64px] h-[48px] md:h-[64px] bg-[#f5f5f5] rounded-full flex items-center justify-center mx-auto mb-[16px] md:mb-[24px]">
            <Heart className="w-[24px] md:w-[32px] h-[24px] md:h-[32px] text-[#6b6b6b]" />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[16px] md:gap-[24px]">
          {wishlistedProducts.map((p) => (
            <div
              key={p.id}
              className="bg-white border-2 border-[#e5e5e5] rounded-[16px] md:rounded-[20px] p-[12px] md:p-[16px] flex flex-col justify-between hover:border-[#1F4D3A] hover:shadow-md transition-all group"
            >
              {/* Image */}
              <div className="relative aspect-square w-full bg-[#f5f5f5] rounded-xl overflow-hidden mb-[12px] md:mb-[16px]">
                ...
                <button
                  ...
                  className="absolute top-[8px] md:top-[12px] right-[8px] md:right-[12px] w-[28px] md:w-[32px] h-[28px] md:h-[32px] bg-white rounded-full flex items-center justify-center text-rose-500 hover:bg-rose-50 shadow-sm transition-all border border-[#e5e5e5]"
                >
              ...
              <div className="text-left space-y-[6px] md:space-y-[8px] flex-1 flex flex-col justify-between">
                <div>
                  <span className="text-[10px] md:text-[11px] text-[#C9A227] font-bold uppercase tracking-wider">
                    {p.category}
                  </span>
                  <h3
                    onClick={() => onNavigateProduct(p.id)}
                    className="font-playfair text-[15px] md:text-[16px] font-bold text-[#1a1a1a] hover:text-[#1F4D3A] transition-colors cursor-pointer line-clamp-1 mt-[2px] md:mt-[4px]"
                  >
                    {p.name}
                  </h3>
                  <div className="flex items-center gap-[4px] py-[2px] md:py-[4px]">
                    ...
                    <span className="text-[11px] md:text-[12px] font-bold text-[#1a1a1a]">
                      {p.rating}
                    </span>
                  </div>
                </div>

                <div className="pt-[12px] md:pt-[16px] border-t border-[#e5e5e5] flex items-center justify-between mt-[12px] md:mt-[16px]">
                  <span className="font-bold text-[15px] md:text-[16px] text-[#1F4D3A]">
                  ...
                  <button
                    ...
                    className="w-[36px] h-[36px] bg-[#0a0a0a] hover:bg-[#1F4D3A] text-white rounded-full flex items-center justify-center transition-colors shadow-sm"
                  >
