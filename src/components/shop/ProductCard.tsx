...
export const ProductCard: React.FC<ProductCardProps> = ({ 
  ...
  if (viewMode === "list") {
    return (
      ...
      >
        <div className="relative w-full md:w-[32%] aspect-square md:aspect-[3/4] max-h-[280px] md:max-h-none bg-[#fafafa] flex-shrink-0 flex items-center justify-center border-b md:border-b-0 md:border-r border-[#f0f0f0] overflow-hidden">
          <motion.img
            src={primaryImage}
            alt={product.name}
          ...
          <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5 items-start">
            ...
            {showBestSellerBadge && (
              <Badge className="bg-white/70 backdrop-blur-sm text-[#1F4D3A]">Best Seller</Badge>
            )}
            {showNewBadge && (
              <Badge className="bg-[#1F4D3A] text-white">New</Badge>
            )}
          </div>
        </div>
        ...
        <div className="p-[24px] flex-1 flex flex-col justify-between space-y-4 bg-white">
          <div className="space-y-[12px]">
            <span className="text-[10px] uppercase tracking-[2px] text-[#C9A227] font-bold block">
              {product.category || "HAIR CARE"}
            </span>
            <h3 className="font-playfair text-[24px] font-bold text-[#1a1a1a] leading-tight group-hover:text-[#1F4D3A] transition-colors duration-300 tracking-[-0.01em]">
              {product.name}
            </h3>
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    ...
                    className={`w-[14px] h-[14px] ${
                      i < Math.floor(product.rating) ? "fill-[#C9A227] text-[#C9A227]" : "text-[#e5e5e5] fill-[#e5e5e5]"
                    }`}
                  />
                ))}
              </div>
              <span className="text-[12px] font-bold text-[#1a1a1a]">{product.rating.toFixed(1)}</span>
              <span className="text-[12px] text-[#6b6b6b]">({product.reviewCount})</span>
            </div>

            <p className="text-[14px] text-[#6b6b6b] font-light leading-[1.7] line-clamp-2 max-w-2xl">
              {product.description}
            </p>
          </div>
          ...
          <div className="pt-[20px] border-t border-[#f0f0f0] flex flex-wrap items-center justify-between gap-[16px]">
            <div className="space-y-[4px]">
              <div className="flex items-baseline gap-[8px]">
                <span className="text-[20px] font-bold text-[#1F4D3A]">
              ...
              <div className="flex items-center gap-[6px]">
                ...
                <span className="text-[11px] font-bold uppercase tracking-[1px] text-[#6b6b6b]">
                  {isOutOfStock ? (
            ...
            <div className="flex items-center gap-[12px] w-full sm:w-auto">
              <button
                ...
                className="w-[44px] h-[44px] shrink-0 transition-colors flex items-center justify-center cursor-pointer text-[#1F4D3A] hover:text-[#C9A227]"
                title="Quick View"
              >
              ...
              <button
                ...
                className={`w-[44px] h-[44px] shrink-0 transition-colors cursor-pointer flex items-center justify-center ${
                  isFavorited ? "text-red-500" : "text-[#1F4D3A] hover:text-red-500"
                }`}
                title="Wishlist"
              >
              ...
              <button
                ...
                className="flex-1 sm:flex-none bg-[#0a0a0a] hover:bg-[#1F4D3A] text-white disabled:bg-gray-200 disabled:text-[#6b6b6b] font-bold py-[14px] px-[32px] rounded-xl text-[13px] uppercase tracking-[1px] transition-all duration-300 flex items-center justify-center gap-[8px] cursor-pointer whitespace-nowrap"
              >
                {isAdding ? (
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[4/5] max-h-[350px] md:max-h-[420px] bg-gradient-to-b from-[#fafafa] to-[#f5f5f5] flex items-center justify-center overflow-hidden">
        {images.map((img, idx) => (
          <img
            key={idx}
            src={img}
        ...
        {images.length > 1 && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-[6px] z-10">
            {images.map((_, idx) => (
              <div 
                key={idx} 
                className={`rounded-full shadow-sm transition-all duration-300 ${idx === currentImageIdx ? 'w-[8px] h-[8px] bg-[#C9A227]' : 'w-[6px] h-[6px] bg-white'}`}
              />
            ))}
          </div>
        )}
      ...
      <div className="p-[12px] md:p-[16px] px-[16px] md:px-[20px] pb-[16px] md:pb-[20px] flex-1 flex flex-col bg-white text-left">
        <span className="text-[8px] md:text-[9px] uppercase tracking-[2px] font-bold text-[#C9A227] block mb-[4px] md:mb-[6px]">
          {product.category || "HAIR CARE"}
        </span>
        <h3 className="font-playfair text-[12px] md:text-[14px] font-semibold text-[#1F4D3A] leading-snug line-clamp-2 mb-[6px] md:mb-[8px] group-hover:text-[#C9A227] transition-colors duration-300">
          {product.name}
        </h3>
        <div className="flex items-center gap-[4px] md:gap-[6px] mb-[8px] md:mb-[12px]">
          ...
          <span className="text-[9px] md:text-[10px] text-[#6b6b6b]">
            ({product.reviewCount || 128})
          </span>
        </div>

        <div className="flex items-baseline gap-[6px] md:gap-[8px] mb-[12px] md:mb-[16px]">
          <span className="text-[13px] md:text-[15px] font-bold text-[#1F4D3A]">
            Rs. {(product.price || 0).toLocaleString()}
          </span>
          {product.originalPrice && (
            <span className="text-[9px] md:text-[10px] text-[#6b6b6b] line-through">
              Rs. {(product.originalPrice || 0).toLocaleString()}
            </span>
          )}
        </div>
        ...
        {parsedVariants && parsedVariants.length > 0 && (
          <div className="mb-[10px] md:mb-[12px]">
            <span className="text-[9px] md:text-[10px] uppercase tracking-[1px] text-[#6b6b6b] mb-[4px] md:mb-[6px] block">
              Size
            </span>
                ...
                <button
                  ...
                  className={`px-[10px] md:px-[12px] py-[4px] md:py-[6px] rounded-lg text-[9px] md:text-[11px] transition-colors ${
                    selectedSize === size
                      ...
                      : "border-[1.5px] border-[#e5e5e5] bg-white text-gray-600 hover:border-[#C9A227]"
                  }`}
                >
                  {size}
                </button>
        ...
        <div className="mt-auto">
          <button
            onClick={handleQuickAdd}
            disabled={isOutOfStock || isAdding}
            className="w-full bg-[#0a0a0a] hover:bg-[#1F4D3A] text-white disabled:bg-gray-200 disabled:text-[#6b6b6b] font-bold py-[12px] px-[12px] rounded-xl text-[11px] flex items-center justify-center gap-[8px] uppercase tracking-[1px] cursor-pointer border-none transition-all duration-300 active:scale-[0.98]"
          >
            {isAdding ? (
